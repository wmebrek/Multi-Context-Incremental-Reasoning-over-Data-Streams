...
object ProgramTicker {
  ...
  case class Config(programFiles: Seq[File] = null,
                    reasoner: ReasonerChoice = ReasonerChoice.incrementalAtms,
    ...
    def buildReasoner(program: LarsProgram): Reasoner = {
      ...
      val preparedReasoner = reasoner match {
        ...
        case ReasonerChoice.incrementalAtms => reasonerBuilder.configure().withIncrementalAtms().use()
  ...
  def connectIO(engine: Engine, config: ProgramTicker.Config): Unit = {
