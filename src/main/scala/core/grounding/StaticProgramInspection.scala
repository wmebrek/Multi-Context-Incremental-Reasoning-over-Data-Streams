...
case class StaticProgramInspection[TRule <: Rule[THead, TBody], THead <: HeadAtom, TBody <: ExtendedAtom](rules: Seq[TRule]) extends ProgramInspection[TRule,THead,TBody] {
  ...
  // Method to add a rule to the existing rules and return a new instance of StaticProgramInspection
  def addRule(rule: TRule): StaticProgramInspection[TRule, THead, TBody] = {
    StaticProgramInspection(rules :+ rule)
  }
