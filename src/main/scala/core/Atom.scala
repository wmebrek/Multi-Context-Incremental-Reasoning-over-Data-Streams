...
/**
  ...
  */
sealed trait Atom extends HeadAtom with Serializable{
...
case class Predicate(caption: String) extends Serializable{
...
trait GroundAtom extends Atom with Serializable{
  override def isGround(): Boolean = true

  override def assign(assignment: Assignment): Atom = this
}

object Falsum extends GroundAtom with Serializable{
...
case class ContradictionAtom(predicate: Predicate) extends GroundAtom with Serializable{
  override def toString = predicate.toString
}

case class PredicateAtom(predicate: Predicate) extends GroundAtom with Serializable{
  override def toString = predicate.toString
}

trait AtomWithArguments extends Atom with Serializable{
...
object AtomWithArguments extends Serializable{
...
trait NonGroundAtom extends AtomWithArguments with Serializable

object NonGroundAtom extends Serializable{
  def apply(predicate: Predicate, arguments: Seq[Argument]) = NonGroundAtomWithArguments(predicate, arguments)
}

case class NonGroundAtomWithArguments(override val predicate: Predicate, arguments: Seq[Argument]) extends NonGroundAtom with Serializable{
...
case class GroundAtomWithArguments(override val predicate: Predicate, arguments: Seq[Value]) extends GroundAtom with AtomWithArguments with Serializable{
  override def isGround() = true
}

object GroundAtom extends Serializable{
...
trait PinnedAtom extends AtomWithArguments with Serializable{
