...
object ReasonerChoice extends Enumeration {
  ...
  val incremental, incrementalAtms, clingo = Value
}
...
case class ArgumentBasedConfiguration(config: Configuration) {
  ...
  def buildEngine(reasonerChoice: ReasonerChoice,
    ...
    } else if (reasonerChoice == ReasonerChoice.incrementalAtms) {
      val atms = Atms()
      val reasoner = config.configure().withIncrementalAtms().withAtms(atms).use().seal()
