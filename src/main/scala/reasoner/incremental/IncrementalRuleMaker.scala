...
case class IncrementalRuleMaker(larsProgramEncoding: LarsProgramEncoding, grounder: Pregrounder = Pregrounder()) {
  ...
  def incrementalRules(tick: Tick, signal: Option[Atom]): Seq[ExpiringRule] = {
    ...
    val expiringRules: Seq[ExpiringRule] = {
      if (timeIncrement) { pin(rulesToPinForTimeIncrement ++ ruleWithVariableOntheFly) }
      else {
        val durations: Seq[RuleWithDuration] = prepare(__base_rules_with_duration, signal)
        ruleWithVariableOntheFly = ruleWithVariableOntheFly ++ durations
        pin(rulesToPinForCountIncrement ++ durations )
      }
    }
  ...
  private def prepare(xRules: Seq[AnnotatedNormalRule], signal: Option[Atom] = None): Seq[RuleWithDuration] = groundPartially(xRules, signal) map normalizeTickVariables

  private def groundPartially(xRules: Seq[AnnotatedNormalRule], signal: Option[Atom] = None): Seq[RuleWithDuration] = xRules flatMap { xr =>
    val rwd = xr.asInstanceOf[RuleWithDuration]
    grounder.groundPartially(rwd.rule, signal) map { groundRule =>
      rwd match {
  ...
  private def nrm(atom: Atom): Atom = {
    if (atom.isGround()) { atom }
      ...
      val newArgs = aa.arguments map {
        ...
        case v@StringVariable(name) => if (name == CountPinVariableName) v else StringVariable(name)
        case arg => arg
      }
  ...
  var ruleWithVariableOntheFly: Seq[RuleWithDuration] = Seq()
