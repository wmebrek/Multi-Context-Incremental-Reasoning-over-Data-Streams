...
case class IncrementalReasonerAtms(incrementalRuleMaker: IncrementalRuleMaker, atms: Atms) extends Reasoner {

  //incrementalRuleMaker.staticGroundRules.filter(_.isFact).foreach(atms.add(_))
  //incrementalRuleMaker.staticGroundRules.filter(r => !r.isFact).foreach(atms.add(_))
  incrementalRuleMaker.staticGroundRules.foreach(r => {
    if("xx".equals(r.head.toString)){
      val positives = r.pos.map(atom => atom.toString).mkString(",")
      val negatives = r.neg.map(atom => "-"+atom.toString).mkString(",")
      atms.recorder.contradict(positives, negatives)
    }
    else atms.add(r)
  })
  ...
  override def evaluate(timepoint: TimePoint): Result = {
    ...
    updateToTimePoint(timepoint)
    val maybeAtomsResult = atms.getModel()
    println("ATMS - current model" + maybeAtomsResult.toString)
    //println("ATMS - nogood" + atms.tms.getNogoodSets)
    Result(maybeAtomsResult)
  }
  ...
  def addIncrementalRule(rule: NormalRule): Unit = {
    ...
    /** PROPAGATE
    atms.tms.getJustifications.forEach(j =>
      if(j.getConsequent.getId.equals(rule.head.toString) || true){// || j.getAntecedents.){
        atms.tms.propagate(j,null, Label.getEmptyEnvironment())
      })*/
  }
